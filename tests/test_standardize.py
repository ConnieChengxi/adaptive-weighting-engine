    ...
    panel = pd.DataFrame(
        {
            ...
            "symbol": ["A", "B", "A", "B"],
            "momentum_3m": [1.0, 3.0, 2.0, 4.0],
        }
    )

    result = cross_sectional_zscore(panel, ["momentum_3m"])

    assert "momentum_3m_z" in result.columns
    jan_values = result.loc[result["Date"] == pd.Timestamp("2024-01-31"), "momentum_3m_z"].round(6).tolist()
